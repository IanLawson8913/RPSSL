...
class Human < Player
  def set_name
    ...
    self.name = n
    # Better way to give comp player access to human name? IE without class variable?
...
class Move < RPSGame
  ...
  def vs(other_move)
    return 'human' if WINS[self.move].include?(other_move.to_s)
    return 'computer' if WINS[other_move.to_s].include?(self.move)
  end
